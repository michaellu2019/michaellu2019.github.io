  ...
  return (
    <div className="project-content-container" id="ninja-project-content-container">
      <div className="project-content-container-section project-overview">
        <h1>Overview</h1>
        <p>Made for the MIT class 6.881/6.4212 - Robotic Manipulation, this project involved creating a simulation of a KUKA iiwa robot arm wielding a giant katana to slice fruits thrown at it in real-time. All software was written in Python, leveraging Drake (created by Prof. Russ Tedrake, the course's instructor), a Python/C++ toolbox to build robotic control systems and analyze resulting dynamics.</p>
        <img src={ninjaCover} />
      </div>

      <div className="project-content-container-section project-ideation">
        <h1>Ideation</h1>
        <img src={ninjaFruitNinja} />
        <p>The idea for the project was inspired by the mobile game <a target="_blank" href="https://en.wikipedia.org/wiki/Fruit_Ninja">"Fruit Ninja"</a> that was popular in the 2010s. In this game, fruits would by thrown up in the air, and the player would wield a sword to slice them in mid-air for points. This project aimed to replicate that gameplay in a Drake simulation with a robot arm.</p> 
        <p>In the simulation, fruits would be randomly generated and launched in the air in 3D space. A simulated KUKA iiwa robot arm model with a katana would then detect the fruit in slice it in real time.</p>
      </div>
      
      <div className="project-content-container-section project-design">
        <h1>Design</h1>
        <img src={ninjaDiagram} />
        <p>In the simulation, fruits would be randomly generated and launched in the air. A KUKA iiwa robot arm would have a katana welded to its final linkage, and it would use a camera system to detect the moving fruit, predict its trajectory, then use optimization methods to create a trajectory to cut the fruit as it fell.</p>
        <img src={ninjaTrajectoryDiagram} />
        <p>Once a fruit was detected at a certain position with a certain velocity vector, kinematic equation for the ballistic trajectory of the fruit were solved to predict the future (<i>x</i>, <i>y</i>) position of the fruit for a certain height <i>z</i> as well as what time <i>t</i> at which this would occur. After generating this predicted point <i>P</i>, a linear trajectory of three points was then made with <i>P</i> being the center point, <i>P<sub>0</sub></i> being the current position of the blade, and <i>P<sub>f</sub></i> being an extrapolated point along the vector from <i>P<sub>0</sub></i> to <i>P</i>. One important thing to note was that <i>P</i> and <i>P<sub>f</sub></i> had reference frames calculated and stored with them so that it was known how to orient the blade perpendicular to the fruit's surface to cut the fruit.</p>
        
        <p>Given the linear trajectory above, two optimization problems were formulated for two different approaches for generating a joint-space trajectory to get the robot arm to cut the fruit.</p>
        <div className="image-next-to-container">
          ...
          <div className="image-container"><img src={ninjaIK} /></div>
        </div>
        <div className="image-next-to-container">
          ...
          <div className="image-container"><img src={ninjaKTO} /></div>
        </div>
      ...
      <div className="project-content-container-section project-programming">
        <h1>Programming</h1>
        <p>Drake offered a lot of built-in tools to set up the KUKA iiwa robot controls and physics simulations; since there were no detailed models in literature or in Drake for fruit cutting, fruits were modeled as two half spheres traveling at the exact same velocity as to appear as one body until they collided with another object, at which point they'd separate, simulating a slice.</p>
        <p>Drake also made it easy to solve optimization problems. By setting up the kinematic trajectory optimization and inverse kinematic problems and invoking the Drake SNOPT solver, I was able to get the robot arm to cut arbitrary points in 3D space.</p>    
        <video controls>
          <source src={ninjaDojoVideo} type="video/mp4" />
          Your browser does not support the video tag.
        </video>
        <p>Combining this with the linear trajectory generator above, I was able to get the robot arm to slice through a point in 3D space, represented by the purple point where the fruit would be. The final part was to work on the timing of the trajectory execution so that the robot arm would hit the fruit just as it passed through the predicted (purple) point. This was done by taking time <i>t</i> for when the fruit would pass through the predicted cut point and comparing it to the time <i>t<sub>c</sub></i> when the katana would pass through that point. The execution of the trajectory would then be delayed by <i>t - t<sub>c</sub></i>, essentially having the robot wait to execute the trajectory so that it would intercept the fruit.</p>
        <img src={ninjaTrajectory} />
      </div>
      
      <div className="project-content-container-section project-results">
        <h1>Results</h1>
        <p>With all these systems combined, the robot arm was able to reliably cut fruit thrown in the air, as shown in the video below. The four cuts were made with trajectories from kinematic trajectory optimization, and the last four cuts were made with trajectories from inverse kinematics.</p>
        <video controls>
