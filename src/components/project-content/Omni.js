  ...
  return (
    <div className="project-content-container" id="omni-project-content-container">
      ...
      <div className="project-content-container-section project-ideation">
        ...
        <img src={omniInverseKinematics} />
        <p>Now, given a desired velocity we want to move the robot in, if we solve for our wheel speeds by obtaining the inverse of <i>F<sub>k</sub></i> and multiplying it to both sides of the equaiton, we can obtain the necessary motor speeds to get that desired velocity, thus solving for the inverse kinematics of the system.</p>
      ...
      <div className="project-content-container-section project-design">
        ...
        <img src={omniCAD} />
        <p>IG42 24VDC 340 RPM Gear Motors with Encoders and a 1:17 reduction ratio were chosen to achieve the desired 2 m/s speed and 0.5 m/s<sup>2</sup> acceleration. The desired speed and torque values calculated from those requirements and then ensured to be within the motor's torque-speed curve operating region with some safety margin.</p>
        <img src={omniCurve} />
      </div>
      
      <div className="project-content-container-section project-fabrication">
        <h1>Fabrication</h1>
        <p>The 8020 frame made much of the fabrication process relatively painless with the drop-in T nuts, but initial testing revealed certain drop-in T nuts (fasteners that would drop into the 8020 channel and tighten parts together through friction) near the drive motors were coming loose due to vibrations from the omni-wheels propogating upwards. In these areas, close-fit holes were drilled so that the drop-in T nuts would be replaced with long 1/4-20 bolts and lock nuts to keep the frame and motor mounting brackets secured together.</p>
        <img src={omniChassis} />
        <p>The Intel RealSense camera was mounted to the top of the robot with a 3D-printed mount, and a plexiglass cover was placed over the frame on which electroncis could be mounted. An STM32-based microcontroller was chosen for its good compute power, clock speed, and the available motor driver shields that the lab produced.</p>
        <div className="image-next-to-container">
      ...
      <div className="project-content-container-section project-programming">
        <h1>Programming</h1>
        <p>Since the robot's ability to move in any direction depended on a precise amount of speed from each motor, developing a robust speed-controller for the motors was essential. This was achieved by reading the quadrature encoder values of the motor and turning those values it to a measured speed of the motor. There would be a setpoint speed to run the motor at (based on the math above), so the error between the setpoint and measured value would be calculated and fed into a feedback controller.</p>    
        ...
        <div className="image-next-to-container">
          <div className="image-container"><img src={omniKp} /></div>
          <div className="image-container"><img src={omniKpKd} /></div>
        </div>
      </div>
